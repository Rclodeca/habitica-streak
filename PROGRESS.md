...
**UI, post-redesign:** `CharacterPanel.vue` shows the 4 equip slots as a 96×96 2×2 icon grid (same footprint as the sprite) sitting to the sprite's right; hovering or tapping a slot shows a name/bonus callout (tap uses a pinned-slot ref since touch doesn't fire `:hover`). Both `CharacterPanel.vue` and `BossPanel.vue` now display their full stat breakdown (damage stats / equipped items for the character; attack/armor/resist for the boss) inline, always — the old tap-to-open `CharacterStatsModal.vue`/`BossStatsModal.vue` are deleted, since nothing they showed is tap-gated anymore. Level/EXP/current-health are omitted from the inline stat list since the heading and bars already show them.
